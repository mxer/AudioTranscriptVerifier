...
def create_fileids(audiodir,dirlist,train_fileid,mfc_fileids_file):
    ...
    for dir in dirlist:
        ...
        for root,dirs,files in os.walk(os.path.join(audio_dir,"train_audio")):
            for file in files:
                if file.endswith('.raw'):
                    print(file)
                    arg = audiodir + u"%s" %(os.path.join(dir,"train_audio"))+os.path.sep+"%08d\n" %(filecounter)
                    f.write(arg)
                    mfdir = os.path.join(audio_dir,"train_mfc")
                    if not os.path.exists(mfdir):
                        os.makedirs(mfdir)
                    arg = audiodir + u"%s" %(os.path.join(dir,"train_mfc"))+os.path.sep+"%08d\n" %(filecounter)
                    mf.write(arg)
                    filecounter += 1
                    global_counter += 1
    f.close()
...
def create_transcripts(transfile,super_prompts_file,trfile,scriptlist,dirlist,transdir):
    ...
    for file in scriptlist:
        filename = transdir + os.path.sep + file
        print(filename)
        fcounter = 0
...
#                print(line)
                f.write("<s> %s </s> "%(line) + "(%s"%(filepath)+os.path.sep+"%08d)\n" % (fcounter))
                tf.write("<s> %s </s> "%(line)+"(%s"%(filepath)+os.path.sep+"%08d)\n" % (fcounter))
                sf.write("%s\n" %(line))
                fcounter += 1
        index += 1
    f.close()
